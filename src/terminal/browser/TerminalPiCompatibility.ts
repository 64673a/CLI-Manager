...
export interface PiTerminalContext {
  projectTool: string;
  startupCmd: string;
  titleTool: string;
  outputHint?: string;
...
export interface PiTerminalDiagnostics {
  ...
  updateContext(context: PiTerminalContext): void;
...
const markerPreview = (text: string): string | null => {
  const markerIndex = text.indexOf(PI_DIAGNOSTIC_MARKER);
  if (markerIndex < 0) return null;
  const start = Math.max(0, markerIndex - DIAGNOSTIC_PREVIEW_RADIUS);
  const end = Math.min(
    text.length,
    markerIndex + PI_DIAGNOSTIC_MARKER.length + DIAGNOSTIC_PREVIEW_RADIUS,
  );
  return sanitizePreview(text.slice(start, end));
};

const summarizeText = (text: string, state: TextSummaryState) => {
...
const summarizeBuffer = (terminal: Terminal) => {
...
export const createPiTerminalDiagnostics = (
  sessionId: string,
  ...
  return {
    ...
    updateContext(context) {
      if (!enabled) return;
      piActive = piActive || isPiTerminalContext(context);
      diagnosticActive = diagnosticActive || piActive;
    },
    onFrame(frame, rawText, normalizedText) {
      if (!enabled) return;
      if (!piActive || !diagnosticActive) {
        const detectionText = `${detectionTail}${rawText}`;
        piActive = piActive || hasPiOutputSignature(detectionText);
        diagnosticActive = diagnosticActive || (
          piActive || detectionText.includes(PI_DIAGNOSTIC_MARKER)
        );
        detectionTail = detectionText.slice(-DIAGNOSTIC_TAIL_LIMIT);
      }
