...
- **Claude / Codex 历史转换**：生成新会话的非破坏性转换不再复用删除、恢复操作的目标进程排他检查；Codex 正在运行时也可执行 Claude -> Codex 转换，共享 JSONL 索引改为单次追加写，SQLite 注册继续等待真实写锁。转换后继续对话时，若目标 CLI 没有独立项目配置、但原 `cwd` 唯一命中项目目录，会直接执行目标 CLI 裸恢复命令，不再弹出项目选择框或泄漏原 CLI 启动配置。删除与备份恢复仍保持运行态保护。
