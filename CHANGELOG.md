...
- **分屏 Pane 状态标记**：当前焦点 Pane 使用终端主题强调色；主会话 Hook 的完成、错误与审批状态使用可配置颜色，完成状态默认使用 `#51A0CC`，并在窗口失焦、最小化或进入托盘后仅保留后台状态线。标记严格以终端内容区为边界，不包围 Pane Tab 栏；终端设置新增完整边框、顶部标记与 Tab 框线三种样式及颜色选择器，配置默认启用、持久化并参与偏好同步。
