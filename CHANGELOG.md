...
- **分屏 Pane 状态标记**：仅在当前可见布局确实包含多个 Pane 时显示，单 Pane 即使有多个 Tab 也不画线。当前焦点 Pane 默认使用 `#51A0CC` 边框色；主会话 Hook 的完成、错误与审批状态使用可配置颜色，并在窗口失焦、最小化或进入托盘后仅保留后台状态线。标记严格以终端内容区为边界，不包围 Pane Tab 栏；终端设置新增完整边框、顶部标记与 Tab 框线三种样式及颜色选择器，配置默认启用、持久化并参与偏好同步。
